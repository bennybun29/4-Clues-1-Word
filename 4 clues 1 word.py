...
score_tasks = {
    ...
    300: "I DARE YOU TO DARE SOMEONE",
...
# Finish Background
finish_bg = pygame.image.load(r"C:\Users\user\Downloads\finish_bg.png")
finish_bg = pygame.transform.scale(finish_bg, (WIDTH, HEIGHT))
finish_bg_rect = finish_bg.get_rect(topleft=(0, 0))

#Wrong Background
wrong_bg = pygame.image.load(r"C:\Users\user\Downloads\wrong_bg.png")
wrong_bg = pygame.transform.scale(wrong_bg, (WIDTH, HEIGHT))
wrong_bg_rect = wrong_bg.get_rect(topleft=(0, 0))
...
def playing_screen():
    ...
    score_text = font.render(f"Score: {score}", True, STORM)
...
def show_correct_message(message, delay=2000):
    screen.fill(WHITE)
...
def reveal_answer():
    delay = 10000
    screen.fill(WHITE)
    screen.blit(wrong_bg, wrong_bg_rect)
    
    answer_text = font.render(f"The correct answer is: {current_word}", True, PINK)
...
def end_game():
    ...
    # Display the message
    text = font.render("You FINISHED the GAME!", True, RED)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                ...
                if current_guess.lower() == current_word:
                    show_correct_message("Correct answer!", delay=1000)
                    generate_word()
                    attempts = 3
                    score += 10
                    if not word_clue_pairs_copy:
